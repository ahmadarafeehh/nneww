...
// Add this to your PostCard class definition
...
class _PostCardState extends State<PostCard>
  ...
  void initState() {
    ...
    // Initialize with the current count, but immediately refresh with accurate count
  ...
  // SIMPLE COUNT: Direct counting without complex unwrapping
  ...
  // FAST COMMENTS COUNT: Direct Supabase response handling
  Future<void> _fetchCommentsCount() async {
    try {
      // Direct queries - Supabase responses are already Lists
      ...
      // Direct counting - Supabase returns List directly
      ...
      // Fallback to original count if there's an error
      if (mounted) {
        setState(() {
          _commentCount = (widget.snap['commentsCount'] ?? 0).toInt();
        });
  ...
  // FIXED: FAST RATINGS FETCH - Fixed the null return issue
  Future<void> _fetchInitialRatings() async {
    try {
      // Single query to get all rating data at once
      ...
      if (mounted) {
        setState(() {
          _totalRatingsCount = response.length;
          ...
          if (user != null) {
            // FIXED: Use proper null-safe approach
            Map<String, dynamic>? userRating;
            try {
              userRating = response.firstWhere(
                (r) => r['userid'] == user.uid,
              );
            } catch (e) {
              userRating = null;
            ...
            if (userRating != null) {
              _userRating = (userRating['rating'] as num).toDouble();
              _showSlider = false;
            } else {
              _userRating = null;
              _showSlider = true;
  ...
  // OPTIMIZED RATING HANDLER - Same as ImageViewScreen
  void _handleRatingSubmitted(double rating) async {
    ...
    setState(() {
      ...
      // Optimistic update - same as ImageViewScreen
  ...
  // Build video player without pause button
      ...
      child: _isVideo
          ...
          : InteractiveViewer(
              ...
              child: Image.network(
                ...
                width: double.infinity,
                height: double.infinity,
                // Add errorBuilder to prevent encoding errors from showing
  ...
  Widget _buildRightActionButtons(_ColorSet colors) {
    return Column(
      children: [
        // User Profile Picture - Make it clickable
        GestureDetector(
  ...
  Widget _buildBottomOverlay(model.AppUser user, _ColorSet colors) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Rating Section - Using the same smooth rating system as ImageViewScreen
          RatingSection(
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: _isLoadingRatings
            ? // Skeleton placeholder for ratings loading
            Container(
                width: 100,
                height: 24,
                decoration: BoxDecoration(
    ...
    return Stack(
      ...
      children: [
        IconButton(
          ...
          onPressed: () {
            // Don't pause video - let it continue playing in bottom half
            // Call the onCommentTap callback to open comments
...
// In PostCard class, update the _openCommentsPanel method:
...
// FAST RATING LIST SCREEN: Optimized version
...
class _FastRatingListScreenState extends State<FastRatingListScreen> {
  ...
  // FAST USER CACHE: Same pattern as search screen
  ...
  // FAST INITIAL RATINGS: Single query and bulk user fetch
  Future<void> _fetchInitialRatings() async {
  ...
  // FAST USER BULK FETCH: Same pattern as search screen
  Future<void> _bulkFetchUsers() async {
    final Set<String> userIds = {};

    // Collect all user IDs from ratings
