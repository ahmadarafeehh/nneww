...
// Helper method to parse timestamp
  ...
  Widget _buildNotificationSkeleton(_NotificationColorSet colors) {
    return ListView.builder(
      padding: const EdgeInsets.symmetric(vertical: 16),
      itemCount: 8, // Show 8 skeleton items
      itemBuilder: (context, index) {
        // Alternate between different notification types for realism
        if (index % 4 == 0) {
          return _buildFollowNotificationSkeleton(colors);
        } else if (index % 4 == 1) {
          return _buildCommentNotificationSkeleton(colors);
        } else if (index % 4 == 2) {
          return _buildRatingNotificationSkeleton(colors);
        } else {
          return _buildFollowRequestSkeleton(colors);
        }
      },
    );
  }

  Widget _buildFollowNotificationSkeleton(_NotificationColorSet colors) {
    return Container(
  ...
  Widget _buildCommentNotificationSkeleton(_NotificationColorSet colors) {
    return Container(
      margin: const EdgeInsets.symmetric(vertical: 6, horizontal: 16),
      decoration: BoxDecoration(
        color: colors.cardColor,
  ...
  Widget _buildRatingNotificationSkeleton(_NotificationColorSet colors) {
    return Container(
      margin: const EdgeInsets.symmetric(vertical: 6, horizontal: 16),
      decoration: BoxDecoration(
        color: colors.cardColor,
  ...
  Widget _buildFollowRequestSkeleton(_NotificationColorSet colors) {
    return Container(
      margin: const EdgeInsets.symmetric(vertical: 6, horizontal: 16),
      decoration: BoxDecoration(
        color: colors.cardColor,
  ...
  Widget _buildEmptyStateSkeleton(_NotificationColorSet colors) {
    return Center(
      child: Column(
  ...
  // -------------------------
  // End Enhanced Placeholder Methods
  // -------------------------
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return _buildNotificationSkeleton(widget.colors);
    }

    if (_notifications.isEmpty) {
      return _buildEmptyStateSkeleton(widget.colors);
    }
...
// ... (rest of the code remains exactly the same - _NotificationItem class and all notification type widgets)
