...
class _ImageViewScreenState extends State<ImageViewScreen> {
  ...
  bool _viewRecorded = false; // ADDED: View recording flag
  ...
  void initState() {
    ...
    _loadBannerAd();
    _recordView(); // ADDED: Record view on initialization
  ...
  // ADDED: View recording method
  Future<void> _recordView() async {
    if (_viewRecorded) return; // Prevent duplicate recordings
  ...
  Future<void> _fetchCommentsCount() async {
    try {
      // Fetch comments count (top-level comments)
      final commentsRes = await Supabase.instance.client
          .from('comments')
          .select('id')
          .eq('postid', widget.postId);

      // Fetch replies count (all replies for this post)
  ...
  // NEW: TikTok/Reels-style comments navigation
  void _navigateToComments(_ImageViewColorSet colors) {
    // Show TikTok-style bottom sheet comments
    // Video continues playing in the background - no pausing
    showModalBottomSheet(
      ...
      builder: (context) => CommentsBottomSheet(
        ...
        isVideo: _isVideo,
        onClose: () {
          // No need to resume since video never stopped
        },
      ),
    ).then((_) {
      _fetchCommentsCount(); // Refresh comment count when sheet closes
    });
  }
                          ...
                          children: [
                            IconButton(
                              ...
                              onPressed: () => _navigateToComments(
                                  colors), // Updated to use new method
                            ),
                            if (commentLen > 0)
                              Positioned(
                                top: -6,
